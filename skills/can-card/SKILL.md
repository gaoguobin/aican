...
加载此 skill 时，先读 `${CLAUDE_SKILL_DIR}/user-config.json`。

- **存在** → 使用其中的 device_type / baudrate / channel / dbc_path
- **不存在** → 用 AskUserQuestion 询问上述 4 项，写入该文件
...
3. **波特率要匹配** — 与被测设备一致
4. **所有工具都需要 device_type 参数** — 从 user-config.json 取值
