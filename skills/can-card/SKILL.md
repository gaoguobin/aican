...
1. **检查设备状态** → `list_open_devices()` 确认设备是否已打开
2. **打开设备** → `open_device("{device_type}")`
3. **初始化通道** → `init_channel("{device_type}", channel={channel}, baudrate={baudrate})`
4. **加载 DBC**（如需信号解析）→ `load_dbc("{dbc_path}")`。若 dbc_path 为空，先询问用户路径
5. **收发数据**（见下方场景）
6. **关闭设备** → `close_device("{device_type}")`（结束时）

> 上述 `{device_type}`、`{channel}`、`{baudrate}`、`{dbc_path}` 均取自 user-config.json 中的值。

## 常用场景

### 接收 CAN 报文
```
clear_buffer("{device_type}", channel={channel}) → receive("{device_type}", channel={channel}, filter_ids=[目标ID])
```
**关键：接收前务必 clear_buffer！** 缓冲区会积压旧帧，不清除会读到过期数据。

### 读取 DBC 信号值
```
load_dbc("{dbc_path}") → read_signal("信号名关键词", "{device_type}", channel={channel})
```
read_signal 内部已自动处理 clear_buffer + receive + 解码。

### 发送 CAN 报文
```
send_can("{device_type}", can_id=0x100, data=[0x01, 0x02, ...], channel={channel})
```
扩展帧需加 `is_extended=True`。CANFD 设备可用 `send_canfd` 发送最多 64 字节。

### 查看缓冲区状态
```
get_receive_count("{device_type}", channel={channel})
```
...
4. **所有工具调用都需要 device_type 参数** — 从 user-config.json 中取值
